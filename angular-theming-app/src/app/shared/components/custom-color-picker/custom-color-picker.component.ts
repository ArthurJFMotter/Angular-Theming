...
export class CustomColorPickerComponent {
  ...
  readonly activeCustomColors = this.themeService.activeCustomColors;
  readonly activeProfile = this.themeService.activeProfile;
  readonly isProfileMode = computed(() => this.themeService.scheme().startsWith('profile-'));
  ...
  constructor() {
    // FIX: This effect tracks the active scheme. If the component opens during a 
    // click race-condition, it will automatically resync the drafts the millisecond 
    // the scheme finishes updating!
    effect(() => {
      const currentScheme = this.themeService.scheme();
  ...
  onSaveProfile(): void {
    ...
    if (this.isProfileMode()) {
      this.themeService.updateActiveProfileName(name);
      this.actionComplete.emit();
    } else {
      this.themeService.saveCurrentAsProfile(name);
      this.actionComplete.emit();
    }
  }

  onDeleteProfileConfirm(): void {
    this.themeService.deleteActiveProfile();
    this.actionComplete.emit();
  }
}